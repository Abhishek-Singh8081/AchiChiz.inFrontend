...
function CustomizeCategory() {
  ...
  try {
    ...
    const featuredProducts = data.data
      .filter(p => p.isFeatured === true) // ✅ Keep only featured products
      .map((p, idx) => {
        let color = '#cccccc';
    ...
    setApiProducts(featuredProducts); // ✅ Only featured ones are passed to AllProducts
    setLoading(false);
  } catch (err) {
      ...
      {showMobileFilter && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <div className="absolute left-0 top-0 h-full w-80 bg-white shadow-lg transform transition-transform duration-300 ease-in-out">
            <div className="p-4 border-b  mt-3 border-gray-200 flex justify-between items-center">
